...
def toeplitz_score(matrix):
    """
    Calculate how 'Toeplitz-like' a matrix is using PyTorch tensors.
    Returns a score between 0 and 1, where 1 means perfect Toeplitz and 0 means maximally non-Toeplitz.
    
    Parameters:
    matrix (torch.Tensor): Input matrix to evaluate
    
    Returns:
    torch.Tensor: Toeplitz score between 0 and 1
    dict: Additional metrics and statistics
    """
    ...
    # Use exponential decay to map variance to [0,1] where 0 means high variance (non-Toeplitz)
    max_possible_variance = torch.var(matrix.flatten())
    
    if max_possible_variance == 0:
        normalized_score = torch.tensor(1.0 if total_variance == 0 else 0.0)
    else:
        normalized_score = torch.exp(-total_variance / max_possible_variance)
    
    # Additional metrics
    metrics = {
        'toeplitz_score': normalized_score.item(),
        'total_diagonal_variance': total_variance.item(),
        'max_diagonal_variance': max([stats['variance'] for stats in diagonal_stats.values()], default=0),
        'diagonal_stats': diagonal_stats,
        'interpretation': {
            'perfectly_toeplitz': normalized_score.item() > 0.99,
            'highly_toeplitz': normalized_score.item() > 0.9,
            'moderately_toeplitz': normalized_score.item() > 0.7,
            'weakly_toeplitz': normalized_score.item() > 0.5,
            'non_toeplitz': normalized_score.item() <= 0.5
        }
    }
...
if __name__ == "__main__":
    ...
    plt.plot(losses)
    plt.savefig("loss_curve_toeplitz.png")
    plt.clf()
